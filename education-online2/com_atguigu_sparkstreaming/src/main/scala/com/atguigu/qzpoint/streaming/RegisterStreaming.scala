...
object RegisterStreaming {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName(this.getClass.getSimpleName)
      .set("spark.streaming.kafka.maxRatePerPartition", "100") //设置kfaka每次消费的条数 ，必须设置项，不然sparkStreaming容易出现OOM
      .setMaster("local[*]")
    ...
    val kafkaMap: Map[String, Object] = Map[String, Object](
      ...
      "key.deserializer" -> classOf[StringDeserializer], //kafka内部的存储是二进制类，我们需要转换成可以使用的类型
      "value.deserializer" -> classOf[StringDeserializer],
      "group.id" -> groupid,
      "auto.offset.reset" -> "earliest",
      "enable.auto.commit" -> (false: lang.Boolean)
    )
    ssc.checkpoint("hdfs://nameservice1/sparkstreaming/checkpoint")
