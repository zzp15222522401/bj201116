...
object KuduSparkTest5 {
  def main(args: Array[String]): Unit = {
    ...
    val tableFields = (Array(new StructField("id", IntegerType, false),
      new StructField("serviceType", StringType), new StructField("count", LongType)))
    val arrayList = new util.ArrayList[String]()
    arrayList.add("id")
    val b = new CreateTableOptions().setNumReplicas(1).addHashPartitions(arrayList, 3)
    kuduContext.createTable("register_table", StructType(tableFields), Seq("id"), b)
  }
}
